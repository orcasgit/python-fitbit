...
class Repository():
    ...
    def get_intraday(self):
        ...
        self.firestore.store_intraday(data=steps, date=self.date, doc_name="steps")
        self.csv.store_intraday(data=steps, date=self.date, doc_name="steps")

        calories = self.fitbit.intraday_time_series(
            resource="calories")
        self.rdb.store_intraday(data=calories, date=self.date, doc_name="calories")
        self.firestore.store_intraday(data=calories, date=self.date, doc_name="calories")
        self.csv.store_intraday(data=calories, date=self.date, doc_name="calories")
        
        distance = self.fitbit.intraday_time_series(
            resource="distance")
        self.rdb.store_intraday(data=distance, date=self.date, doc_name="distance")
        self.firestore.store_intraday(data=distance, date=self.date, doc_name="distance")
        self.csv.store_intraday(data=calories, date=self.date, doc_name="calories")

        heart = self.fitbit.intraday_time_series(
            resource="heart")
        self.rdb.store_intraday(data=heart, date=self.date, doc_name="heart")
        self.firestore.store_intraday(data=heart, date=self.date, doc_name="heart")
        ...
        try:
            elevation = self.fitbit.intraday_time_series(
                ...
                end_date=self.date)
            self.rdb.store_intraday(data=elevation, date=self.date, doc_name="elevation")
            self.firestore.store_intraday(data=elevation, date=self.date, doc_name="elevation")
        ...
        try:
            floors = self.fitbit.intraday_time_series(
                ...
                start_date=self.date,
                end_date=self.date)
            self.rdb.store_intraday(data=floors, date=self.date, doc_name="floors")
            self.firestore.store_intraday(data=floors, date=self.date, doc_name="floors")
